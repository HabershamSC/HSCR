(() => {
  ...
  function validateConfiguration() {
    if (!state.runtime || typeof state.runtime !== "object") {
      throw new Error("config/runtime.js did not define HAM_RUNTIME.");
    }
    if (!Object.keys(state.fields).length) {
      throw new Error("config/fields.js did not define any fields.");
    }
    if (!Object.keys(state.taxonomies).length) {
      throw new Error("config/taxonomies.js did not define any taxonomies.");
    }
    if (!Object.keys(state.profiles).length) {
      throw new Error("config/profiles.js did not define any profiles.");
    }
  }
  ...
  async function loadGeoJson() {
    ...
    if (!configuredUrl) {
      throw new Error("No GeoJSON data URL is configured in config/runtime.js or the active profile.");
    }
  ...
  function loadGoogleMaps() {
    ...
    if (!apiKey || apiKey.includes("REPLACE_WITH")) {
      throw new Error(
        "A Google Maps browser API key has not been configured. Open config/runtime.js and replace the placeholder with the browser key from the current working map."
      );
    }
  ...
  function refreshMap(options = {}) {
    ...
    for (const record of state.records) {
      ...
      if (visible) {
        ...
        if (!isBlank(labelText) && record.center) {
  ...
  function updateFeatureStatus(visibleCount, baseVisibleCount) {
    ...
    const details = [];
    const publishedAt = state.metadata.publishedAt || state.metadata.generatedAt;
    if (publishedAt) {
